...
namespace Legalworks\IsbnTools\Sources\Exlibris;
...
class Client
{
    ...
    public function __construct()
    {
        $this->client = Http::withOptions([
            'base_uri' => 'http://bibsys-network.alma.exlibrisgroup.com/view/sru/47BIBSYS_NETWORK',
            'headers' => [
            ...
            'query' => [
                ...
                'maximumRecords' => 10, //1-100
                'startRecord' => 0, //1-99.000
            ]
        ]);
    }
    ...
    public function first(string $query)
    {
        ...
        // dd($xml->text('/srw:*/srw:version'));

        // $service = new Service();
        // $service->elementMap = [
        //     '{http://example.org/}response' => 'Sabre\Xml\Element\KeyValue',
        // ];

        // dd($service->parse($response->body()));
        $xml = $response->body();
        $xml = json_decode(json_encode(simplexml_load_string($xml)));


        dd($xml->records->record->recordData->dc, $response->body());
    }
    ...
    protected function query($query)
    {
        ...
        $xml = QuiteSimpleXMLElement::make($response->body(), self::$nsPrefixes);

        $version = $xml->text('/srw:*/srw:version');

        $test = $xml->first('//srw:record/srw:recordData');
        dd($test, $response->body());
    }
